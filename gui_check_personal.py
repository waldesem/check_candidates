...
import check_main, menu_upload, menu_download, about_me
...
class MainApp():
    def __init__(self, master):
        ...
        self.command_lst = [self.click_upload, self.click_check, 
                                menu_download.download_file, about_me.about_programm]
        ...
        self.txt_ank = ['Должность', 'Подразделение', 'Фамилия Имя Отчество', 'Предыдущее ФИО', 'Дата рождения', 'Место рождения', 
        ...
        for l in range (len(self.txt_chk)):
            Label(self.tab_check, text=f"{self.txt_chk[l]}", font=('Arial', 10), width=40, anchor='w', 
