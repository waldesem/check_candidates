...
def db_response():
    ...
    try:
        ...
        try:
            if record[0][1] == fio_search.get() and record[0][2] == dr_search.get():
                for i in range(3):
                    create_labels(tab_db,txt_db[i], 40, 0, i+2)
                    create_labels(tab_db,record[0][i], 60, 1, i+2)
        except IndexError:
...
def take_info():
    ...
    for i in record[0]:
        pdf.add_page()
        pdf.image(i)
    pdf.output("/home/semenenko/MyProjects/yourfile.pdf", "F")
...
if __name__ == "__main__":
    #проверка логина и пароля
        ...
        menu_label_lst = ['Загрузить анкету', 'Начать проверку', 'Получить заключение', 
                        'Искать в базе данных', 'Выгрузить запись', 'О программе']
        command_lst = [click_upload, click_check,
                        menu_download.download_file, db_response, take_info, about_me.about_program]
        ...
        txt_db = ['ID', 'Фамилия Имя Отчество', 'Дата рождения' ]
