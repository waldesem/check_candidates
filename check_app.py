...
from database_app import response_db, SQL, CONNECT
...
def check():
    ...
    data_inn = {
        ...
        "bplace": "",
        "doctype": '21',
        "docno": f'{anketa_values[7][:2]} {anketa_values[7][2:]} {anketa_values[8]}',
    ...
    query = f"SELECT * FROM candidates WHERE full_name like {anketa_values[2]} and birthday like {anketa_values[4]}"
    ...
    query = f"SELECT * FROM disqual_fns WHERE G2 like {anketa_values[2].upper()} and G3 like {anketa_values[3]}"
    if response_db(connect, query):
    ...
    query = f"SELECT * FROM list_of_expired_passports WHERE PASSP_SERIES = {anketa_values[7]} and PASSP_NUMBER = {anketa_values[8]}"
    if response_db(connect, query):
